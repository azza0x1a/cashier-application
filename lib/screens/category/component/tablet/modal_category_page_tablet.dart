...
class ModalCategoryPageTablet extends StatefulWidget {
  const ModalCategoryPageTablet({super.key});

  @override
  State<ModalCategoryPageTablet> createState() => _ModalCategoryPageTabletState();
}

class _ModalCategoryPageTabletState extends State<ModalCategoryPageTablet> {
  String userId = '';
  String userName = '';
  ...
  void insertInvtItemCategory(context) async {
    // Remove data for the 'counter' key.
    final prefs = await SharedPreferences.getInstance();
    showLoaderDialog(context);
    token = prefs.getString('token')!;
